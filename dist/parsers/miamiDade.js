...
--------------------------------------------------------- */
function normalizeMiamiDadeOwnerNames(owners) {
...
/* ---------------------------------------------------------
   MIAMI-DADE PARSER
--------------------------------------------------------- */
export function parseMiamiDadePA(html) {
    const data = {};
    /* ---------------------------------------------------------
       FOLIO NUMBER
  ...
  --------------------------------------------------------- */
    const ownerMatches = html.matchAll(/<div[^>]*class="pa-font-size-11"[^>]*>([^<]+)<\/div>/gi);
    const owners = [];
  ...
  --------------------------------------------------------- */
    const propertyAddressBlock = html.match(/<div[^>]*class="property-add[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
    if (propertyAddressBlock) {
        const spans = Array.from(propertyAddressBlock[1].matchAll(/<span[^>]*>([^<]+)<\/span>/gi), m => m[1].trim());
    ...
    /* ---------------------------------------------------------
      MAILING ADDRESS
    --------------------------------------------------------- */
    const mailingBlock = html.match(/<td[^>]*class="pi_mailing_address"[\s\S]*?<div[^>]*>([\s\S]*?)<\/div>/i);
    let mailingStreet;
    let mailingCity;
    let mailingZip;
    if (mailingBlock) {
        ...
        const spans = Array.from(block.matchAll(/<span[^>]*>([^<]+)<\/span>/gi), m => m[1].trim());
        // Street: first span
    ...
    --------------------------------------------------------- */
    const legalBlock = html.match(/<div[^>]*class="table-responsive legal-descr"[\s\S]*?<table[\s\S]*?<\/table>/i);
    if (legalBlock) {
        const spans = Array.from(legalBlock[0].matchAll(/<span[^>]*>([\s\S]*?)<\/span>/gi), m => m[1]
