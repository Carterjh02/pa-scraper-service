...
export function parseSeminolePA(html) {
  ...
  --------------------------------------------------------- */
    const addressBlockMatch = html.match(/<div class="parcel-address">\s*([^<]+?)\s*<\/div>/i);
    ...
    --------------------------------------------------------- */
    const ownerMatches = html.matchAll(/<div class="parcel-address">\s*([A-Z0-9 ,.&']+)\s*<\/div>/gi);
  ...
  --------------------------------------------------------- */
    const legalMatch = html.match(/<td[^>]*>\s*<div[^>]*>([\s\S]*?)<\/div>\s*<\/td>/i);
    if (legalMatch) {
    ...
    --------------------------------------------------------- */
    const imageMatch = html.match(/<img[^>]+src="([^"]+footprintimage[^"]+)"/i);
    if (imageMatch) {
