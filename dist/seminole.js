...
--------------------------------------------------------- */
export async function extractSeminoleAssets(page, address) {
    /* ---------------------------------------------------------
    ...
    }, addressTabSelector, { timeout: 20000 });
    ...
    await page.waitForFunction((text) => Array.from(document.querySelectorAll("td"))
        .some(td => td.innerText.includes(text)), targetRowText, { timeout: 30000 });
    ...
    /* ---------------------------------------------------------
       8. EXTRACT PARCEL ID
    --------------------------------------------------------- */
    let parcelMatch = html.match(/Parcel Number:\s*([\d\-]+)/i) ||
